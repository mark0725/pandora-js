...
export function renderContent(basePath:string, item: MenuItem) {
    ...
    if (item.type === "iframe" && item.url) {
        return <IframeLayout key={basePath} url={item.url} title={item.title || ""} />
    }

    if (item.type === "pan-page") {
        return <PanPageview key={basePath} id={item.id || ""} path={basePath} menu={item} />
    }

    if (item.type === "nav-page") {
        return <NavPageLayout key={basePath} id={item.id || ""} path={basePath} menu={item}/>
    }

    if (item.type === "subnav-page") {
        return <SubNavPageLayout key={basePath} id={item.id || ""} path={basePath} menu={item} />
    }

    if (item.type === "sidebar-page") {
        return <SidebarPageLayout key={basePath} id={item.id || ""} path={basePath} menu={item} />
    }

    if (item.type === "setting-page") {
        return <SettingPageLayout key={basePath}  id={item.id || ""} path={basePath} menu={item} />
    }
    if (item.type === "dashboard-page") {
        return <DashboardPageLayout key={basePath}  id={item.id || ""} path={basePath} menu={item} />
    }

    if (item.type === "select-nav-page") {
        return <SelectNavPageLayout key={basePath}  id={item.id || ""} path={basePath} menu={item} />
    }
